...
test=0;
rm -rf experiments/single_stage_amp/
    ...
    for selectivePressure in 1 5 10 15; do
        ...
        for ES in comma plus; do
            ...
            for fitness in bestFit idealSine maxAmp; do
                ...
                avg=$(bc -l <<< "$avg / 10");
                echo "experiment $test: $path" | tee -a experiments/single_stage_amp/results.txt;
                echo "average: $avg" | tee -a experiments/single_stage_amp/results.txt;
            done
        done
    done
done
