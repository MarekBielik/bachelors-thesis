...
test=0;
rm -rf results.txt experiments/

for mu in 1 5 10 15; do
    for selectivePressure in 1 5 10 15; do
        lambda=$((mu * selectivePressure));

        #pick the right number of generations to for the stop condition
        ...
        for ES in comma plus; do
            ...
            for fitness in bestFit idealSine maxAmp; do
                avg=0;
                ((test++));

                for i in {1..10}; do
                    path="experiments/mu$mu/lambda$lambda/$ES/$fitness/$i";
                    mkdir -p ${path};
                    ...
                    echo ${result} | tee -a experiments/results.txt;
                    avg=$(bc -l <<< "$avg + $result");
                done

                avg=$(bc -l <<< "$avg / 10");
                echo "experiment $test: $path" | tee -a experiments/results.txt;
                echo "average: $avg" | tee -a experiments/results.txt;
            done
        done
    done
done
