        ...
        for ES in comma plus; do
            ...
            for fitness in idealSine maxAmp; do
                ...
                for i in {1..10}; do
                    ...
                    mkdir -p ${path};
                    result=`./build/bt -o ${path} --mu=${mu} --lambda=${lambda} --max_gen=20000 --stop_gen=${stop_gen} --print_gen=1 --print_change=0.95 --ES=${ES} --max_res=200000 --fitness=${fitness} --amplitude=6 --max_diff=25 --two_stage_amp`;
